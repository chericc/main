...
数据传送指令把数据从原位置复制到目的位置。传送的两个操作数不能都指向内存位置。

传送指令主要有4条，分别是`moveb`、`movew`、`movel`、`moveq`，这些指令都执行相同的操作，主要区别在于它们操作的数据大小不同：分别是1字节、2字节、4字节和8字节。
...
```c
#include <cstdint>
int exchange(int32_t *xp, int32_t y)
{
        int32_t x = *xp;
        *xp = y;
        return x;
...
其中第一条move指令将寄存器`%rdi`对应内存的值传送到寄存器`%eax`中，对应代码中取出`xp`指针指向的值并作为返回值；第二条move指令将寄存器`%esi`中的值传送到寄存器`%rdi`对应的内存的值，对应代码中将`y`的值赋值到`%xp`指针指向的值。

#### 3.4.4 压入和弹出数据

压栈和出栈操作可以将数据压入程序栈中，以及从程序栈中弹出数据。栈指针`%rsp`保存着栈顶元素的指针。`pushq`指令的功能是把四字数据压入到栈上，而`popq`指令是弹出四字数据。这些指令都只有一个操作数：压入的数据源和弹出的数据目的。

将一个四字值压入栈中，首先要将栈指针减8，然后将值写到新的栈顶地址。

指令`push %rbp`的等价于以下汇编代码：
...
注：目前查到的资料中，并没有说明压栈或出栈后栈指针的偏移量。书中提到的4字节压入后栈指针减8，可能意味着所有栈元素都是以8字节为单位的。这种情况，即使压入和弹出的数据大小不一致，也不影响后续操作的数据正确性（因为一旦弹出错误，就会引起栈指针错误）。和硬件结构相关，通常认为64bit结构下，pop和push指令每次加减8。
...
| `add S,D`  | $D\leftarrow D+S$       | 加(add S with D to D)        |
| `sub S,D`  | $D\leftarrow D-S$       | 减(subtract S from D to D)   |
| `imul S,D` | $D\leftarrow D\times S$ | 乘                           |
| `xor S,D`  | $D\leftarrow D \land S$ | 异或                         |
| `or S,D`   | $D\leftarrow D\mid S$   | 或                           |
| `and S,D`  | $D\leftarrow D\& S$     | 与                           |
| `SAL k,D`  | $D\leftarrow D<< k$     | 左移                         |
| `SHL k,D`  | $D\leftarrow D<<k$      | 左移                         |
| `SAR k,D`  | $D\leftarrow D>>_{A}k$  | 算术右移(A means arithmetic) |
| `SHR k,D`  | $D\leftarrow D>>_{L}k$  | 逻辑右移                     |

#### 3.5.1 加载有效地址

加载有效地址指令`leaq`实际上是`movq`指令的变形。它的第一个操作数看上去是一个内存引用，但该指令并不是从指定的位置读入数据，而是将有效地址写入到目的操作数。

`leaq`指令还可以用来简洁地描述普通的算术操作。例如，如果`%rdx`的值为$x$，那么指令`leaq 7(%rdx,%rdx,4),%rdx`将设置寄存器`%rdx`的值为$5x+7$。注：第一个操作数为地址$5x+7$对应的内存中的值，而取其地址就是$5x+7$。

注：`leaq`指令被用来做简单运算的原因是复杂的寻址方式引入了算术运算，而一些情况下，寻址过程中的计算并不需要ALU单元的参与，这意味着这个指令更加的高效。

#### 3.5.2 一元和二元操作

一元操作只有一个操作数，既是源又是目的。这个操作数可以是一个寄存器，也可以是一个内存位置。

二元操作有两个操作数，其中，第二个操作数既是源又是目的。
