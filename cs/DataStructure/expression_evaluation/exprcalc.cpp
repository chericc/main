 ...
 */
int ExprCalc::infix2sufix(const std::list<ExprItem>& infixitems,
                    ...
                    case ExprItem::BrType::RightBr: {
                        while (!stackOp.empty()) {
                            ...
                            if (!(crefTop.eType == ExprItem::ItemType::BR &&
                                  crefTop.data.eBrType ==
                                      ExprItem::BrType::LeftBr)) {
...
int ExprCalc::priority(const ExprItem& itema, const ExprItem& itemb) {
    ...
    // };
    static const int map[7][7] = {
        0, 0, -1, -1, 1, -1, 9, 0, 0, -1, -1, 1,  -1, 9,  1,  1, 0,
        0, 1, -1, 9,  1, 1,  0, 0, 1, -1, 9,  -1, -1, -1, -1, 0, -1,
        9, 1, 1,  1,  1, 1,  0, 9, 9, 9,  9,  9,  9,  9,  9,
    };
    int ida = id(itema);
    int idb = id(itemb);
    assert(ida >= 0 && ida <= 7);
