 ...
 * part and right part.
 */
std::size_t select_nth_helper_group(std::vector<int>& vec, std::size_t nBegin,
                                    std::size_t nEnd, std::size_t nSeperator) {
    /**
...
int select_nth_helper(std::vector<int>& vec, std::size_t nBegin,
                      std::size_t nEnd, std::size_t nNth) {
...
int select_nth_v2(std::vector<int>& vec, size_t nNth) {
    ...
    while (nSeperator != nNth) {
        std::size_t nNewSeperator =
            select_nth_helper_group(vec, nBegin, nEnd, nSeperator);
...
int main() {
    ...
    TIME_POINT_BEGIN(D);
    std::nth_element(vecBackup.begin(),
                     vecBackup.begin() + vecBackup.size() / 2, vecBackup.end());
    TIME_POINT_END(D);

    std::cout << "value=" << nNthValue << " " << vecData[vecData.size() / 2]
              << std::endl;
    std::cout << "value=" << nNthValueA << " "
              << vecBackupA[vecBackupA.size() / 2] << std::endl;
