...
/**
 * @brief Group vec into two parts. Elements of left part are all smaller than
 * vec[nSeperator], and elements of right part are all not smaller than
 * vec[nSeperator]. The vec[nSeperator] will be put int the middle beween left
 * part and right part.
 * @return Return the index where seperator is located.
 */
std::size_t quick_sort_helper_group(std::vector<int>& vec,
                                    std::size_t nSeperator, std::size_t nBegin,
                                    std::size_t nEnd) {
...
void quick_sort_helper(std::vector<int>& vec, std::size_t nBegin,
                       std::size_t nEnd) {
    ...
    int nSeperatorNew =
        quick_sort_helper_group(vec, nSeperatorIndex, nBegin, nEnd);
