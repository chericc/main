...
TreeNode::TreeNode()
    :key(),lnode(nullptr),rnode(nullptr),parent(nullptr)
{}
...
TreeNode::TreeNode(const KEY_TYPE &_key, TreeNode *_lnode, TreeNode *_rnode, TreeNode *_parent)
...
std::size_t BSTree::height() const
{
    return max_depth__(m_rootnode.rnode);
}
