...
由LCS的特征，可以把求两个串Xm、Yn的LCS分解为子问题，如下：

1. 若这两个串Xm、Yn的最后一个元素相等，则这个元素为LCS的尾部元素。然后求 Xm-1 和 Yn-1 的LCS。
将得到的LCS附加得到的尾部元素作为最终的 LCS。
2. 若这两个串Xm、Yn的最后一个元素不等，则LCS一定是 Xm-1 和 Yn 或 Xm 和 Yn-1 的LCS。

将前缀串的LCS求解结果暂存下来，避免反复求解，这就是动态规划核心。
