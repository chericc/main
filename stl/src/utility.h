...
void PRINT_ELEMENTES(const std::map<KEY, VALUE> container,
...
void PRINT_ELEMENTES_OP(const T& container, _Op function,
                        const std::string& optstr = "") {
    std::cout << optstr << std::endl;
...
void PRINT_ELEMENTES_OP2(const T1& c1, const T2& c2, _Op op,
                         const std::string& optstr = "") {
    ...
    for (; cit1 != c1.end(); cit1++, cit2++) {
        std::cout << "OP2(" << *cit1 << "," << *cit2 << ")=" << op(*cit1, *cit2)
                  << std::endl;
    }
}
}  // namespace TEST
