...
void TEST::test_regular_expression() {
    ...
    /* subexpression */
    {
        ...
        cout << "bool found = regex_search(str, smatch, regex(reg_str))"
             << endl;
        std::smatch smatch;
        ...
        for (unsigned int i = 0; i < smatch.size(); ++i) {
            ...
            cout << "smatch.position(" << i << ")=" << smatch.position(i)
                 << endl;
    ...
    /* subexpression_2 */
    {
        ...
        for (; std::regex_search(pos, end, smatch, reg);
             pos = smatch.suffix().first) {
    ...
    /* regex token iterator */
    {
        ...
        {
            cout << "sregex_token_iterator pos(str.cbegin(), str.cend(), reg, "
                    "{-1,-1,0,0,1,1,2,2,3,3} )"
                 << endl;
            std::regex reg(reg_str);
            std::sregex_token_iterator pos(str.cbegin(), str.cend(), reg,
                                           {-1, -1, 0, 0, 1, 1, 2, 2, 3, 3});
    ...
    /* regex_replace */
    {
        ...
        std::string str_replace =
            std::regex_replace(str, std::regex(reg_str), R"(<$1 value="$2"/>)");
        cout << "str_replace=" << str_replace << endl;

        cout << R"(regex_replace(str, regex, "<\1 value="\2">", format_sed)"
             << endl;
        str_replace =
            std::regex_replace(str, std::regex(reg_str), R"(<\1 value="\2"/>)",
        ...
        str_replace.clear();
        std::regex_replace(std::back_inserter(str_replace), str.cbegin(),
                           str.cend(), std::regex(reg_str),
                           R"(<$1 value="$2"/>)",
