...
void TEST::test_async_future() {
    ...
    // passing argument
    {
        ...
        std::future<int> result(
            std::async(std::launch::deferred, func4, nParam));
...
void TEST::test_thread() {
    ...
    // exception handling
    {
        ...
        cout << "TEST exception handling. Not catching it will lead to "
                "termination"
             << endl;
        try {
            std::thread t1(func3);
...
void func6(std::promise<std::string>& p) { p.set_value("this is amazing!"); }
}  // namespace
...
void TEST::test_mutex_lock() {
    ...
    // mutex lock_guard
    {
        ...
        std::future<void> future1 =
            std::async(std::launch::async, print, "First thread");
        std::future<void> future2 =
            std::async(std::launch::async, print, "Second thread");
        future1.get();
        future2.get();
        cout << endl;
    }
    // mutex lock_guard: lock more than once
    {
        ...
        try {
            std::future<void> future =
                std::async(std::launch::async, callPrint, "First thread");
            future.get();
        } catch (std::system_error& e) {
            cout << "catch exception: e.code().message()=" << e.code().message()
                 << endl;
        }
        cout << endl;
    }
    // recursive_lock lock_guard
    {
        ...
        std::future<void> future1 =
            std::async(std::launch::async, printA, "First thread");
        std::future<void> future2 =
            std::async(std::launch::async, printB, "Second thread");
        future1.get();
        future2.get();
        cout << endl;
    }
