...
void TEST::test_chrono_duration() {
    ...
    // operation
    {
        ...
        // 1h + 30min = 5400s
        std::chrono::seconds durResultInSecond =
            std::chrono::hours(1) + std::chrono::minutes(30);
        cout << "1hour + 30min = " << durResultInSecond.count() << " sec"
             << endl;
        ...
        // 4min / 1s = 240
        cout << "4min / 1s = "
             << std::chrono::minutes(4) / std::chrono::seconds(1) << endl;

        // 4min / 5min = 0
        cout << "4min / 5min = "
             << std::chrono::minutes(4) / std::chrono::minutes(5) << endl;
    ...
    // cast
    {
        ...
        std::chrono::hours durHour =
            std::chrono::duration_cast<std::chrono::hours>(durS);
        std::chrono::minutes durMinute =
            std::chrono::duration_cast<std::chrono::minutes>(
                durS % std::chrono::hours(1));
        std::chrono::seconds durSecond =
            std::chrono::duration_cast<std::chrono::seconds>(
                durS % std::chrono::minutes(1));

        cout << durS.count() << "sec= " << durHour.count() << "hours "
             << durMinute.count() << "minutes " << durSecond.count()
             << "seconds" << endl;
