...
#define _debug(x...)                                                         \
    do {                                                                     \
        printf("[test][debug][%s %d %s]", __FILE__, __LINE__, __FUNCTION__); \
        printf(x);                                                           \
    } while (0)
#define _info(x...)                                                         \
    do {                                                                    \
        printf("[test][info][%s %d %s]", __FILE__, __LINE__, __FUNCTION__); \
        printf(x);                                                          \
    } while (0)
#define _error(x...)                                                         \
    do {                                                                     \
        printf("[test][error][%s %d %s]", __FILE__, __LINE__, __FUNCTION__); \
        printf(x);                                                           \
    } while (0)
#else
#define _debug(x...) \
...
int test_filemap_normal(int nTotalNum, int nTestNum, const char* szFileName) {
    ...
    char szObjFile[64] = {};
    snprintf(szObjFile, sizeof(szObjFile), "test.dat_normal_%d_%d", nTotalNum,
             nTestNum);
    ...
    for (int i = 0; i < nTestNum; ++i) {
        ...
        snprintf(szValue, sizeof(szValue), "value%d_%d", i,
                 i * i); /* 这样更容易碰撞 */
        cMap[szName] = szValue;
    }
    ...
    for (auto it = cMap.cbegin(); it != cMap.cend(); ++it) {
        ...
        snprintf(value.byteData, sizeof(value.byteData), "%s",
                 it->second.data());
    ...
    for (auto it = cErasedSet.cbegin(); it != cErasedSet.cend();
         ++it) { /* 随机删除一半的元素 */
    ...
    /* 删除后对比 */
    for (auto it = cErasedList.cbegin(); it != cErasedList.cend();
         ++it) { /* 被删除元素 */
    ...
    for (auto it = cMapMod.cbegin(); it != cMapMod.cend();
         ++it) { /* 未被删除元素 */
        FILEMAP_KEY key = {};
        snprintf(key.szKey, sizeof(key.szKey), "%s", it->first.data());
        int ret_exist = filemap_existitem(hFileMap, &key);
...
static int test_filemap_reload(int nTotalNum, const char* szFileName) {
    ...
    snprintf(szObjFileName, sizeof(szObjFileName), "%s_%d.dat", __FUNCTION__,
             nTotalNum);
    ...
    for (int nTestCount = 0; nTestCount <= 10;
        ...
        for (int i = 0; i < nAddCount; ++i) {
            ...
            snprintf(key.szKey, sizeof(key.szKey), "name_%d_%d_%d", k, k * k,
                     k * k * k);
            snprintf(value.byteData, sizeof(value.byteData), "value_%d", k);

            auto it = mapBackUp.find(key.szKey);
            if (it == mapBackUp.cend()) { /* 如果是新元素 */
                if ((int)mapBackUp.size() <
                    nTotalNum) { /* 如果没有满，就加进去 */
                    mapBackUp.insert(std::pair<std::string, std::string>(
                        key.szKey, value.byteData));
                    int ret = filemap_setitem(hFileMap, &key, &value);
                    assert(ret == 0);
                } else { /* 如果满了，就会失败 */
                    int ret = filemap_setitem(hFileMap, &key, &value);
...
int test_filemap1() {
    int nNum[] = {
        5,  9,   10,  11,  15,   18,   19,   21,   24,    57,
        60, 100, 158, 950, 1342, 2399, 5104, 8907, 10000, 10001,
    };
    for (int i = 0; i < (int)(sizeof(nNum) / sizeof(nNum[0])); ++i) {
        char szFileName[64] = {};
        snprintf(szFileName, sizeof(szFileName), "filemap_%s_%d.txt",
                 __FUNCTION__, nNum[i]);
