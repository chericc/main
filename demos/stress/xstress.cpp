...
static const char* global_progname = PACKAGE;

/* Implemention of runtime-selectable severity message printing.  */
#define dbg(OUT, STR, ARGS...)                            \
    if (global_debug >= 3)                                \
    fprintf(stdout, "%s: dbug: [%lli] ", global_progname, \
            (long long)getpid()),                         \
        fprintf(OUT, STR, ##ARGS), fflush(OUT)
#define out(OUT, STR, ARGS...)                            \
    if (global_debug >= 2)                                \
    fprintf(stdout, "%s: info: [%lli] ", global_progname, \
            (long long)getpid()),                         \
        fprintf(OUT, STR, ##ARGS), fflush(OUT)
#define wrn(OUT, STR, ARGS...)                                 \
    if (global_debug >= 1)                                     \
    fprintf(stderr, "%s: WARN: [%lli] (%d) ", global_progname, \
            (long long)getpid(), __LINE__),                    \
        fprintf(OUT, STR, ##ARGS), fflush(OUT)
#define err(OUT, STR, ARGS...)                                 \
    if (global_debug >= 0)                                     \
    fprintf(stderr, "%s: FAIL: [%lli] (%d) ", global_progname, \
            (long long)getpid(), __LINE__),                    \
        fprintf(OUT, STR, ##ARGS), fflush(OUT)
        ...
        if (do_vm) {
            switch (pid = fork()) {
                case 0: /* child */
                    ...
                    usleep(backoff);
                    if (do_dryrun) exit(0);
                    exit(hogvm(do_vm_bytes, do_vm_stride, do_vm_hang,
                               do_vm_keep));
                case -1: /* error */
                    err(stderr, "fork failed: %s\n", strerror(errno));
                    break;
...
void worker_init(void) {
#ifdef HAVE_SYS_PRCTL_H
    if (prctl(PR_SET_PDEATHSIG, SIGTERM) == -1) exit(0);
#endif
}
...
int usage(int status) {
    const char* mesg =
        ...
        "     --vm-hang N    sleep N secs before free (default none, 0 is "
        "inf)\n"
        ...
        "Note: Numbers may be suffixed with s,m,h,d,y (time) or B,K,M,G "
        "(size).\n";
