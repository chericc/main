...
static void my_track(const char* file, const unsigned int line,
                     const int func_id, const DMALLOC_SIZE byte_size,
                     const DMALLOC_SIZE alignment, const DMALLOC_PNT old_addr,
...
static void main_dump_states() {
    ...
    dmalloc_get_stats(&heap_low, &heap_high, &total_space, &user_space,
                      &current_alloc, &current_alloc_n, &max_allocated,
                      &max_allocated_n, &max_allocated_one);
    ...
    if (NULL != fp) {
        ...
        fprintf(fp,
                "[%s] "
                "alloc:[low=%p,high=%p,total=%lu,user=%lu,cur=%lu,curn=%lu,max="
                "%lu,maxn=%lu,maxone=%lu]\n",
                str_now, heap_low, heap_high, total_space, user_space,
                current_alloc, current_alloc_n, max_allocated, max_allocated_n,
                max_allocated_one);
        fflush(fp);
        xdebug(
            "[%s] "
            "alloc:[low=%p,high=%p,total=%lu,user=%lu,cur=%lu,curn=%lu,max=%lu,"
            "maxn=%lu,maxone=%lu]\n",
            str_now, heap_low, heap_high, total_space, user_space,
            current_alloc, current_alloc_n, max_allocated, max_allocated_n,
            max_allocated_one);
...
DmallocImp::DmallocImp() : thread__(trd_dmalloc_imp, nullptr) {
