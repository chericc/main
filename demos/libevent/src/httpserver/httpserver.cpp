...
static std::string dump_req_content(struct evhttp_request* req) {
    ...
    do {
        ...
        if (header_s) {
            for (struct evkeyval* header = header_s->tqh_first; header;
...
static int generate_html_of_path(evbuffer* evbuf, const char* root_path,
                                 const char* relative_path, int* error_code) {
    bool error = true;
    DIR* dir = nullptr;
    int fd = -1;
    ...
    do {
        ...
        if (S_ISDIR(st.st_mode)) {
            ...
            // relative_path_with_trailing_slash =
            // encode_url(relative_path_with_trailing_slash);
  ...
  <ul>)",
            ...
            while ((ent = readdir(dir))) {
                ...
                std::string item_path =
                    relative_path_with_trailing_slash + name;
                evbuffer_add_printf(evbuf,
                                    R"(   <li><a href="%s">%s</a>
)",
...
static void send_document_cb(struct evhttp_request* req, void* arg) {
    ...
    do {
        ...
        if (generate_html_of_path(evbuf, sopt->docroot, decoded_uri_path,
                                  &http_error_code) < 0) {
        ...
        std::string whole_path =
            std::string() + sopt->docroot + "/" + decoded_uri_path;
        struct stat st = {};
