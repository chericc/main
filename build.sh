mkdir -p build
cd build

OPENSRC_LIB_PATH=~/opensrc
FFMPEG_PATH=$OPENSRC_LIB_PATH/ffmpeg/build/output
LIVE555_PATH=$OPENSRC_LIB_PATH/live555/live/output
NANOMSG_PATH=$OPENSRC_LIB_PATH/nanomsg/build/output
CARES_PATH=$OPENSRC_LIB_PATH/cares/build/output
GTEST_PATH=$OPENSRC_LIB_PATH/googletest/build/output
BOOST_PATH=$OPENSRC_LIB_PATH/boost
NNG_PATH=$OPENSRC_LIB_PATH/nng/build/output
JPEG_PATH=$OPENSRC_LIB_PATH/libjpeg-turbo/build/output
SQLITE_PATH=$OPENSRC_LIB_PATH/sqlite/build/output
SQLPP11_PATH=$OPENSRC_LIB_PATH/sqlpp11/build/output
SPDLOG_PATH=$OPENSRC_LIB_PATH/spdlog/build/output

CUSTOM_INCS=$CUSTOM_INCS:$FFMPEG_PATH/include
CUSTOM_INCS=$CUSTOM_INCS:$LIVE555_PATH/include
CUSTOM_INCS=$CUSTOM_INCS:$NANOMSG_PATH/include
CUSTOM_INCS=$CUSTOM_INCS:$CARES_PATH/include
CUSTOM_INCS=$CUSTOM_INCS:$GTEST_PATH/include
CUSTOM_INCS=$CUSTOM_INCS:$NNG_PATH/include
CUSTOM_INCS=$CUSTOM_INCS:$JPEG_PATH/include
CUSTOM_INCS=$CUSTOM_INCS:$SQLITE_PATH/include

CUSTOM_LIBS=$FFMPEG_PATH/lib
CUSTOM_LIBS=$CUSTOM_LIBS:$LIVE555_PATH/lib
CUSTOM_LIBS=$CUSTOM_LIBS:$NANOMSG_PATH/lib
CUSTOM_LIBS=$CUSTOM_LIBS:$CARES_PATH/lib
CUSTOM_LIBS=$CUSTOM_LIBS:$GTEST_PATH/lib
CUSTOM_LIBS=$CUSTOM_LIBS:$NNG_PATH/lib
CUSTOM_LIBS=$CUSTOM_LIBS:$JPEG_PATH/lib
CUSTOM_LIBS=$CUSTOM_LIBS:$SQLITE_PATH/lib

CUSTOM_PATHS=$CUSTOM_PATHS\;$LIVE555_PATH
CUSTOM_PATHS=$CUSTOM_PATHS\;$SQLPP11_PATH
CUSTOM_PATHS=$CUSTOM_PATHS\;$SPDLOG_PATH

export CMAKE_INCLUDE_PATH=$CUSTOM_INCS
export CMAKE_LIBRARY_PATH=$CUSTOM_LIBS

cmake .. \
-DCMAKE_PREFIX_PATH=$CUSTOM_PATHS \
-DCMAKE_BUILD_TYPE=Debug \
-DCMAKE_INSTALL_PREFIX=output

make install all $@

echo LD_LIBRARY_PATH=\$LD_LIBRARY_PATH:$CUSTOM_LIBS