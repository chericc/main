...
**本章导读**：自动规划研究如何从初始状态出发，自动构造动作序列或策略以达成目标，是人工智能中连接搜索、逻辑推理与决策的核心领域。本章从经典的 STRIPS/PDDL 表示出发，依次介绍状态空间搜索、图规划（GraphPlan）、层次任务网络（HTN）、非确定性与概率规划，最后建立自动规划与基于模型强化学习之间的桥梁。

**学习目标**：
完成本章的学习后，你应当能够：
- 形式化描述经典规划问题，写出 STRIPS/PDDL 动作的前提条件、添加效果与删除效果；
- 推导并分析前向（progression）与后向（regression）搜索的语义差异与适用条件；
- 构建规划图并正确判定动作互斥与命题互斥，说明 GraphPlan 的完备性与步骤最优性；
- 设计 HTN 方法库，将复合任务递归分解为可执行的原语任务网络；
- 区分强规划、弱规划与强循环规划，并为非确定性问题定义策略；
- 建立 MDP/POMDP 模型并应用值迭代、策略迭代、PBVI 或在线规划进行求解；
- 解释基于模型强化学习中 Dyna、MPC、MCTS、CEM 与可微规划的原理与适用条件。

**预备知识**：第 2 章的启发式搜索（A\*）、第 7 章的马尔可夫决策过程与强化学习基础，以及基本的命题/一阶逻辑与概率论知识。
...
状态空间规划将规划问题直接形式化为在**状态空间图**上进行的搜索问题——节点表示世界状态，有向边表示可执行的（实例化）动作。本节侧重**基本概念**与两种搜索的直观对比，更完整的算法伪代码、启发式设计（$h^{\text{add}}$、$h^{\text{max}}$、$h^{\text{FF}}$、地标启发式）与部分有序规划见 §8.2。
...
- **状态空间爆炸**：设每条正向分支有 $b$ 个适用动作，解的长度为 $d$，则搜索树的规模为 $O(b^d)$。对于典型的 Blocks World（积木世界）问题，$b$ 常可达数十至数百，即使 $d=10$，搜索空间也极其庞大——因此**有效的启发式函数是前向搜索成功的关键**（见 §8.2.3）。
...
- **各自的适用场景**：
    ...
    - **混合策略**：现代高性能规划器通常不纯粹采用单一种策略——例如，FF 使用前向搜索 + 松弛启发式，GraphPlan（见 §8.3）结合了前向传播与后向提取，LAMA 在前向搜索中融合了地标分析。理解各类算法的核心机制，有助于设计更高效的混合规划方案。
...
- **不一致效果（Inconsistent Effects）**：$a_1$ 的删除效果抵消 $a_2$ 的某个添加效果（反之亦然）。形式化地：

  $$\text{Del}(a_1) \cap \text{Add}(a_2) \neq \varnothing \quad \text{或} \quad \text{Del}(a_2) \cap \text{Add}(a_1) \neq \varnothing$$

- **干扰（Interference）**：$a_1$ 删除 $a_2$ 的某个前提条件（反之亦然）。即：

  $$\text{Del}(a_1) \cap \text{Precond}(a_2) \neq \varnothing \quad \text{或} \quad \text{Del}(a_2) \cap \text{Precond}(a_1) \neq \varnothing$$
...
| 互斥类型 | 判别依据 | 适用范围 |
|---|---|---|
| **动作互斥 — 不一致效果** | 一个动作删除另一动作的添加效果 | $A_i$ 中任意两个动作 |
| **动作互斥 — 干扰** | 一个动作删除另一动作的前提条件 | $A_i$ 中任意两个动作 |
...
概率规划可以理解为"给定准确的 MDP 模型，离线计算最优策略"；而强化学习则是在"模型未知或需在线学习"的设定下工作。第 7 章中详细讨论的许多 RL 算法（Q-Learning、SARSA 等）本质上是在无模型情况下逼近概率规划的解。
...
第 7 章已从**学习**视角介绍了 MDP 及 Q-Learning、SARSA 等无模型方法。本章从**规划**视角重新审视 MDP，关键区别在于：

- 规划设定下，$P(s' \mid s, a)$ 和 $R(s, a, s')$ 是**预先给定**的，规划器无需与环境交互
- 值迭代和策略迭代是**离线**、**基于模型**的算法
- 当状态空间过大时，规划需要借助启发式搜索（如 LAO*、RTDP），这些技术与第 7 章的 Dyna 架构形成呼应
...
5. 如果状态/动作空间巨大且**无法完整建模**（如围棋、Atari 游戏），跳出传统的概率规划框架，使用第 7 章的无模型深度强化学习方法
...
1. **背景规划（Background Planning）**：
   ...
   - 类似于第 7 章中 Q-Learning 的策略改进过程不断运行，但不仅基于真实数据，也基于模型数据。
...
蒙特卡洛树搜索（MCTS）的基本原理见第 2 章 §2.5。在 RL 语境下，MCTS 的核心角色在于：
...
- **从规划角度理解 RLHF 中的 PPO**：
  - 在 RLHF（基于人类反馈的强化学习）流程中（见第 7 章 §7.7），奖励模型 $r_\phi(y|x)$ 充当了文本生成任务中的**环境模型**（简化版）。
...
| 规划范式 | 代表算法 | 状态空间 | 适用场景 |
|:---|:---|:---|:---|
| 经典规划（STRIPS / PDDL） | 前向搜索、后向搜索、A\* + 启发式 | 命题状态集合（规模 $2^n$） | 确定性、完全可观测、静态环境 |
| 部分有序规划（POP） | UCPOP, VHPOP, RePOP | 计划空间（不完整规划） | 需并行执行或子规划复用 |
| 图规划（GraphPlan） | GraphPlan, IPP, Blackbox | 分层规划图（命题层 + 动作层） | 需步骤最优的经典规划 |
| 层次任务网络（HTN） | SIPE-2, SHOP/SHOP2, O-Plan | 任务分解空间 | 领域知识丰富且结构化 |
| 非确定性规划 | PRP, MyND, GRENDEL | 状态集合，解为策略 | 动作效果非确定、无概率信息 |
| 概率规划（MDP / POMDP） | 值迭代、策略迭代、PBVI、SARSOP、POMCP | 状态（POMDP 为信念空间） | 有概率模型，随机/部分可观测环境 |
| 基于模型的 RL | Dyna-Q、MPC、MCTS、CEM、MuZero | 学习到的隐式/显式动力学空间 | 模型未知或需在线学习 |
