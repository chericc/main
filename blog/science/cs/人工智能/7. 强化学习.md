...
**本章导读**：本章从马尔可夫决策过程（MDP）出发，系统梳理强化学习的核心概念与算法谱系，涵盖动态规划、蒙特卡洛与时序差分、值函数逼近、策略梯度、深度强化学习，以及离线学习与大模型对齐。全章沿着“模型已知 → 模型未知 → 深度化 → 前沿方法”的主线展开，帮助读者理解各类方法如何在偏差与方差、样本效率与稳定性之间做出权衡。

**学习目标**：
完成本章的学习后，你应当能够：
- 形式化描述 MDP 五元组，并写出状态值、动作值函数及其 Bellman 方程；
- 运用策略迭代与值迭代求解已知模型的 MDP，并比较二者的收敛特性；
- 区分蒙特卡洛与时序差分，推导 TD(0) 与 TD(λ) 的更新公式；
- 比较 SARSA、Q-Learning、Expected SARSA 与 Double Q-Learning 的 on/off-policy 特性；
- 说明 DQN 的经验回放、目标网络以及 Rainbow 各改进组件的作用；
- 推导策略梯度定理，并比较 REINFORCE、Actor-Critic、A2C/A3C 的差异；
- 阐述 PPO、SAC、TRPO、Dreamer 的核心思想、优势与适用场景；
- 概括 AlphaGo 系列与 MuZero 的算法演进，以及离线 RL 与 RLHF 的基本流程。
...
- **MDP 求解分类**：
  - **模型已知（Model-based / Planning）**：当转移概率 P 和奖励函数 R 已知时，用**动态规划（Dynamic Programming）**方法求解——见 §7.2。
  - **模型未知（Model-free / Learning）**：当 P, R 未知时，通过与环境的交互**学习（Reinforcement Learning）**——见 §7.3 及之后。
...
#### 7.3.2 蒙特卡洛方法 (Monte Carlo, MC)

#### 7.3.2.1 核心机制 (Core Mechanism)
...
#### 7.3.2.2 增量更新 (Incremental Update)
...
#### 7.3.2.3 探索难题 (Exploration Challenge)
...
#### 7.3.2.4 蒙特卡洛方法性质 (MC Properties)
...
#### 7.3.3 时序差分学习 (Temporal Difference, TD)
...
#### 7.3.3.3 TD(λ) 与资格迹 (Eligibility Traces)
...
#### 7.3.5 批量更新与收敛性 (Batch Updating & Convergence)
...
#### 7.4.1 在策略 TD 控制：SARSA（State-Action-Reward-State-Action，on-policy）
...
#### 7.4.2 离策略 TD 控制：Q-Learning（off-policy，Watkins 1989）
...
```text
初始化 Q(s, a)（如全 0），设定 α、γ、ε
for 每个 episode:
    初始化状态 s
    while s 不是终止状态:
        依 ε-greedy 从 Q(s, ·) 选择动作 a
        执行 a，观察即时奖励 r 和下一状态 s'
...
#### 7.4.5 双 Q 学习 (Double Q-Learning, van Hasselt 2010)
...
#### 7.5.1 深度 Q 网络 (DQN，Deep Q-Network)
...
#### 7.5.2.1 双 Q 网络 (Double DQN，van Hasselt et al. 2016)
...
#### 7.5.2.3 竞争网络 (Dueling DQN，Wang et al. 2016)
...
#### 7.5.2.6 噪声网络 (Noisy Nets，Fortunato et al. 2017)
...
#### 7.5.3 集成算法：Rainbow（Hessel et al. 2018）
...
#### 7.6.1 蒙特卡洛策略梯度：REINFORCE（Williams, 1992）
...
初始化策略参数 θ，学习率 α
for 每个 episode:
    用 π_θ 采样轨迹 (s_0, a_0, r_1, …, s_T)
    for t = 0 … T-1:
        计算折扣累积回报 G_t = Σ_{k=t}^{T-1} γ^{k−t} · r_{k+1}
        更新 θ ← θ + α · γ^t · ∇_θ log π_θ(a_t | s_t) · G_t
直到收敛
```
...
#### 7.6.4 同步优势演员-评论家 (A2C，Advantage Actor-Critic)
...
#### 7.6.5 异步优势演员-评论家 (A3C，Asynchronous Advantage Actor-Critic, Mnih et al. 2016)
...
- **C. SAC (Soft Actor-Critic)** — Haarnoja et al. 2018
  - **核心思想——最大熵强化学习 (Maximum Entropy RL)**：
    - 目标不再是最大化累积奖励，而是最大化：
      $$J(\pi) = \sum_t \mathbb{E}_{(s_t,a_t)\sim\pi}\!\big[r(s_t,a_t) + \alpha\,\mathcal{H}\big(\pi(\cdot|s_t)\big)\big], \qquad \mathcal{H}\big(\pi(\cdot|s_t)\big) = -\mathbb{E}_{a\sim\pi}\!\left[\log\pi(a|s_t)\right]$$
    - 其中 $\mathcal{H}(\pi(\cdot|s_t))$ 是策略在状态 $s_t$ 下的熵（entropy），上式在采样意义下即 $- \log \pi(a_t \mid s_t)$
...
#### 7.10.1 离线强化学习 (Offline RL / Batch RL)
...
#### 7.10.2 RLHF — 从人类反馈中强化学习 (Reinforcement Learning from Human Feedback)
...
#### 7.10.3 DPO — 直接偏好优化 (Direct Preference Optimization)
...
#### 7.10.4 其他对齐方法 (Other Alignment Methods)
...
| 方法族 | 核心思想 | On/Off-policy | 代表算法（适用场景） |
|---|---|---|---|
| 动态规划（DP） | 已知完整模型，用 Bellman 方程迭代求解值函数与策略 | 与策略相关但无需采样 | 策略迭代、值迭代、异步 DP（小规模已知 MDP） |
| 蒙特卡洛（MC） | 用完整 episode 的实际回报平均估计值函数，无偏高方差 | 可有 on/off-policy 两种 | First-Visit / Every-Visit MC、Off-Policy MC（episodic 任务） |
| 时序差分（TD） | 用一步奖励加自举值估计，在线低方差更新 | 取决于控制算法 | TD(0)、TD(λ)、SARSA、Q-Learning（通用 model-free） |
| 深度值方法 | 神经网络逼近 Q 函数，经验回放 + 目标网络稳定训练 | Off-policy | DQN、Double/Dueling DQN、PER、Rainbow（离散高维状态） |
| 策略梯度 | 直接对策略参数求梯度，提升高回报动作概率 | On-policy 为主 | REINFORCE、Actor-Critic、A2C/A3C（连续/随机策略） |
| 信赖域与剪裁 | 限制新旧策略差异，保证单调或稳定改进 | On-policy | TRPO、PPO（通用首选，含 RLHF） |
| 最大熵方法 | 在最大化回报的同时最大化策略熵，鼓励探索 | Off-policy | SAC（连续控制 SOTA） |
| 世界模型 | 学习环境动态，在隐空间想象中训练策略 | Model-based | DreamerV1/V2/V3（样本稀缺、长时域任务） |
| 搜索与规划 | 将神经网络先验与 MCTS 结合，压缩搜索空间 | Model-based 规划 | AlphaGo、AlphaGo Zero、AlphaZero、MuZero（棋类与通用决策） |
| 探索机制 | 通过乐观、计数、好奇心或信息增益驱动探索 | 可叠加于任意算法 | ε-greedy、UCB、Thompson Sampling、ICM/RND、Go-Explore（稀疏奖励） |
| 离线 RL 与对齐 | 从静态数据或人类偏好中学习，避免在线交互风险 | Off-policy / 偏好学习 | CQL、IQL、Decision Transformer、RLHF、DPO（医疗、机器人、LLM 对齐） |

**关键术语中英对照**

| 中文 | English | 一句话说明 |
|---|---|---|
| 马尔可夫决策过程 | Markov Decision Process (MDP) | 用状态、动作、转移、奖励、折扣描述序贯决策问题的形式化框架 |
| 策略 | Policy | 从状态到动作（或动作分布）的映射 |
| 状态值函数 | State-Value Function | 从某状态出发按策略行动所得的期望折扣回报 |
| 动作值函数 | Action-Value Function | 在某状态先执行某动作后再按策略行动所得的期望折扣回报 |
| 动态规划 | Dynamic Programming (DP) | 在已知模型下迭代求解 Bellman 方程的方法 |
| 蒙特卡洛方法 | Monte Carlo (MC) | 用完整 episode 的实际回报估计值函数 |
| 时序差分 | Temporal Difference (TD) | 结合采样与自举、可在线更新的学习方法 |
| 经验回放 | Experience Replay | 存储并随机重放 transition 以打破样本相关性 |
| 目标网络 | Target Network | 参数延迟更新的网络，用于稳定 TD 目标 |
| 优先级经验回放 | Prioritized Experience Replay (PER) | 按 TD 误差大小优先采样关键 transition |
| 优势函数 | Advantage Function | 动作价值相对状态价值的超出量 A(s,a)=Q(s,a)−V(s) |
| 广义优势估计 | Generalized Advantage Estimation (GAE) | 用指数加权多步 TD 误差在偏差与方差间折中 |
| 策略梯度 | Policy Gradient | 直接对策略参数求期望回报梯度的方法 |
| 信赖域 / 剪裁 | Trust Region / Clipping | 限制策略更新幅度的机制，如 TRPO 的 KL 约束与 PPO 的 clip |
| 最大熵强化学习 | Maximum Entropy RL | 在回报目标中加入策略熵以鼓励探索的框架 |
| 世界模型 | World Model | 学习环境动态并在隐空间中进行规划或想象的方法 |
...
| 直接偏好优化 | Direct Preference Optimization (DPO) | 跳过显式奖励模型、直接从偏好数据优化策略的方法 |
