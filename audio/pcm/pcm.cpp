...
static uint8_t ALawCompressTable[] = {
...
static uint16_t ALawDecompressTable[] = {
...
uint8_t LinearToAlawSample(uint16_t sample) {
    int sign = 0;
    int exponent = 0;
    int mantissa = 0;
    ...
    if (sample >= 0x100) {
        ...
        compressedByte = (uint8_t)((exponent << 4) | mantissa);
    } else {
        compressedByte = (uint8_t)(sample >> 4);
    }
    compressedByte ^= (uint8_t)(sign ^ 0x55);
    return compressedByte;
}

uint16_t AlawToLinearSample(uint8_t sample) {
...
uint8_t LinearToMuLawSample(uint16_t sample) {
    int cBias = 0x84;
    int cClip = 0x7F7B;
...
uint16_t MuLawToLinearSample(uint8_t sample) {
...
std::vector<uint8_t> pcm_s16le_to_alaw(const std::vector<uint16_t>& pcm_s16le) {
    std::size_t size = pcm_s16le.size();
    std::vector<uint8_t> buf(size);

    for (std::size_t i = 0; i < size; ++i) {
        buf[i] = LinearToAlawSample(pcm_s16le[i]);
    }
    return buf;
}

std::vector<uint16_t> pcm_alaw_to_s16e(const std::vector<uint8_t>& pcm_alaw) {
    std::size_t size = pcm_alaw.size();
    std::vector<uint16_t> buf(size);

    for (std::size_t i = 0; i < size; ++i) {
        buf[i] = AlawToLinearSample(pcm_alaw[i]);
    }
    return buf;
}

std::vector<uint8_t> pcm_s16le_to_mulaw(
    const std::vector<uint16_t>& pcm_s16le) {
    std::size_t size = pcm_s16le.size();
    std::vector<uint8_t> buf(size);

    for (std::size_t i = 0; i < size; ++i) {
        buf[i] = LinearToMuLawSample(pcm_s16le[i]);
    }
    return buf;
}

std::vector<uint16_t> pcm_mulaw_to_s16e(const std::vector<uint8_t>& pcm_mulaw) {
    std::size_t size = pcm_mulaw.size();
    std::vector<uint16_t> buf(size);

    for (std::size_t i = 0; i < size; ++i) {
        buf[i] = MuLawToLinearSample(pcm_mulaw[i]);
    }
    return buf;
}
