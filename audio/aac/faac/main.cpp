...
#define debug_timeprint_x(x...) \
	do { \
        ...
        \
        printf ("timestamp[%s %d %s] [%s] ", __FILE__,__LINE__,__FUNCTION__, szResult); \
        printf (x); \
	} while (0)
...
int run(const char *input_file, const char *output_file, 
    const Config *config, int repeat)
{
    ...
    else
    {
        xinfo ("test: vbr=%u, bitrate=%u, samplerate=%u\n",
            config->vbr, config->bitrate, config->samplerate);
    }
...
void test(const char *file_in, const char *file_out, int test_type)
{
    ...
    if (0 == test_type)
    {
        snprintf (file_in_real, sizeof(file_in_real), "%s_%u.pcm",
            file_in, config[0].samplerate);
        snprintf (file_out_real, sizeof(file_out_real), "%s_vbr%u_bitrate%u_default.aac",
            file_out, config[0].vbr, config[0].bitrate);
    ...
    else if (1 == test_type)
    {
        ...
        {
            snprintf (file_in_real, sizeof(file_in_real), "%s_%u.pcm",
                file_in, config[i].samplerate);
            if (config[i].vbr > 0)
            {
                snprintf (file_out_real, sizeof(file_out_real), "%s_vbr%u_bitrate%u_sample%u.aac",
                    file_out, config[i].vbr, config[i].bitrate, config[i].samplerate);
            }
            else
            {
                snprintf (file_out_real, sizeof(file_out_real), "%s_abr%u_bitrate%u_sample%u.aac",
                    file_out, config[i].vbr, config[i].bitrate, config[i].samplerate);
            }
    ...
    else if (2 == test_type)
    {
        ...
        snprintf (file_in_real, sizeof(file_in_real), "%s_%u.pcm",
            file_in, config[0].samplerate);
        snprintf (file_out_real, sizeof(file_out_real), "%s_vbr%u_bitrate%u_repeat%d.aac",
            file_out, config[0].vbr, config[0].bitrate, repeat);
