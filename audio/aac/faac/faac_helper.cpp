...
int FAACHelper::init()
{
    ...
    if ((handle = faacEncOpen(sample_rate, channels, 
            &samples_input, &max_bytes_output)) == nullptr)
    {
        ...
        OutputArgs *output_args)
{
    ...
    if (input->audio_data_size > oneFramePCMSize())
    {
        xerror ("data size over: %u>%u\n",
            input->audio_data_size, oneFramePCMSize());
        return -1;
    }
    ...
    if (output->audio_data_size < maxbytesoutput)
    {
        xerror ("output buffer too small(%u < %lu)\n",
            output->audio_data_size, maxbytesoutput);
        return -1;
    }
...
int FAACHelper::encodeStream(const void *data_input, unsigned int data_input_size,
        void *data_output, unsigned int *data_output_size)
{
    ...
    if (nullptr == data_input
        || nullptr == data_output_size)
    {
        xerror ("input error\n");
        return -1;
    }
        ...
        OutputArgs *output_args)
{
    ...
    ret_encode = faacEncEncode (handle, 
        (int32_t*)input->audio_data, 
    ...
    {
        if (0 == input->audio_data_size
            && input_args->flush_flag)
        {
            xinfo ("Eof\n");
