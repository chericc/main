...
AacFDKAac::AacFDKAac() { _data = std::make_shared<InnerData>(); }
...
int AacFDKAac::Init() {
    ...
    if (aacEncoder_SetParam(handle, AACENC_CHANNELORDER, channel_order) !=
        AACENC_OK) {
    ...
    if (aacEncoder_SetParam(handle, AACENC_TRANSMUX, transporttype) !=
        AACENC_OK) {
    ...
    tmp_pcm_buf_size = aacenc_info.frameLength * PCM_ELEMENT_SIZE *
                       2;  // 2 倍最大帧大小，足够缓存不足的输入帧
...
bool AacFDKAac::IsInit__() const { return _data->init_flag; }
