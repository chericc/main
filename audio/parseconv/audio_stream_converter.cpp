...
AudioStreamConverter::AudioStreamConverter(const AudioStreamConverterConfig &config)
    : config_(config)
{
}

int AudioStreamConverter::convert(const void *src_data, int src_size, void *dst_data, int *dst_size)
{
    int ret = 0;
    ...
    if (nullptr == info.src_data
        || nullptr == info.dst_data)
    {
        LOGE("null pointer");
        return -1;
    }

    if (0 == config_.src_sample_bits
        || 0 == config_.dst_sample_bits)
    {
...
int AudioStreamConverter::PCM2PCM_Resample(const ConvertInfo &info)
{
    do 
    {
        ...
        if (EQUAL(config_.dst_sample_rate * DOWN_SCALE_2, config_.src_sample_rate))
        {
            int i = 0;
            for (i = 0; i < src_sample_num && transfer_samples < dst_sample_num_max; ++i)
            {
                if (0 == i % DOWN_SCALE_2)
                {
...
int AudioStreamConverter::PCM2G711A(const ConvertInfo &info)
{
    return 0;
}

int AudioStreamConverter::PCM2AAC(const ConvertInfo &info)
{
    return 0;
}

int AudioStreamConverter::G711A2PCM(const ConvertInfo &info)
{
    return 0;
}

int AudioStreamConverter::AAC2PCM(const ConvertInfo &info)
{
    return 0;
}