...
WavDemuxer::WavDemuxer(const std::string& filename) : _filename(filename) {
    doLoadFile();
}

WavDemuxer::~WavDemuxer() { doCloseFile(); }
...
int WavDemuxer::readHeader(std::shared_ptr<LoadInfo> info) {
    ...
    if (!has_riff || !has_fmt || !has_data) {
        xlog_err("chunk incomplete(riff=%d,fmt=%d,data=%d)", has_riff, has_fmt,
                 has_data);
        berror = true;
    }

    // Just for read convience
    if ((info->fmt.bits_per_sample % 8 != 0) || info->fmt.num_channels != 1) {
        xlog_err("bits_per_sample=%d, num_channels=%d",
                 info->fmt.bits_per_sample, info->fmt.num_channels);
        berror = true;
    }
