...
using XTimerTaskQueueT =
    std::priority_queue<XTimerTaskCtxPtr, std::vector<XTimerTaskCtxPtr>,
                        XTimerTaskCtxGTCmp>;
...
XTimerID XTimerSimple::createTimer(XTimerTask task, XTimerDuration duration) {
    ...
    do {
        ...
        {
            std::lock_guard<std::mutex> lock_task(
                _d->trd_ctx_steady->mutex_task);
...
int XTimerSimple::destroyTimerOfQueue(XTimerID id,
                                      std::shared_ptr<PerThreadCtx> ctx) {
    bool berror_flag = false;
...
std::shared_ptr<XTimerSimple::PrivateData> XTimerSimple::init() {
    ...
    do {
        ...
        auto lambda_func_steady = [this]() {
            return this->workLoop(this->_d->trd_ctx_steady);
        };
        auto lambda_func_system = [this]() {
            return this->workLoop(this->_d->trd_ctx_system);
        };
        data->trd_ctx_steady->trd_worker =
            std::make_shared<XThread>(lambda_func_steady);
        data->trd_ctx_system->trd_worker =
            std::make_shared<XThread>(lambda_func_system);
    } while (0);
