...
TEST(xutility, xswap_n)
{
    ...
    EXPECT_EQ(xswap(u16a), u16b);
    EXPECT_EQ(xswap(u16b), u16a);

    uint32_t u32a = 0x12345678;
    uint32_t u32b = 0x78563412;

    EXPECT_EQ(xswap(u32a), u32b);
    EXPECT_EQ(xswap(u32b), u32a);

    uint64_t u64a = 0x1234567887654321;
    uint64_t u64b = 0x2143658778563412;

    EXPECT_EQ(xswap(u64a), u64b);
    EXPECT_EQ(xswap(u64b), u64a);
}