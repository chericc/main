...
int ImageInterface::SetToTwoColor(const Pixel& refLightPixel,
                                  const Pixel& refDarkPixel,
                                  const Pixel& refMidPixel) {
    int nPixelCount = 0;
    ...
    for (std::size_t i = 0; i < m_vectPixelArray.size(); ++i) {
        int nCurLight = m_vectPixelArray[i].R + m_vectPixelArray[i].G +
                        m_vectPixelArray[i].B;
...
bool ImageInterface::GuessImageType(const char* szFilename,
                                    IMAGE_FILE_TYPE_E* eType) {
    bool bGuessOk = false;

    const uint8_t jpg_file_head[] = {0xff, 0xd8, 0xff};
    const uint8_t png_file_head[] = {0x89, 0x50, 0x4e, 0x47,
                                     0x0d, 0xa0, 0x1a, 0x0a};
    ...
    do {
        ...
        if (memcmp(jpg_file_head, byteFileBuf, sizeof(jpg_file_head)) == 0) {
            *eType = IMAGE_JPEG;
            bGuessOk = true;
        } else if (memcmp(png_file_head, byteFileBuf, sizeof(png_file_head)) ==
                   0) {
            *eType = IMAGE_PNG;
            bGuessOk = true;
        } else if (memcmp(bmp_file_head, byteFileBuf, sizeof(bmp_file_head)) ==
                   0) {
            *eType = IMAGE_BMP;
            bGuessOk = true;
        } else {
