...
int ImageView::ok() const { return (_state ? true : false); }
...
std::shared_ptr<ImageView::State> ImageView::init(
    int w, int h, int depth, std::vector<uint8_t> init_pixel) {
...
uint8_t* ImageView::pixelAt(int x, int y, int offset_pixels) {
    ...
    if (x < 0 || x >= _state->w || y < 0 || y >= _state->h) {
        return nullptr;
    }
