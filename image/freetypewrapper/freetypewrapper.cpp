...
FreeTypeWrapper::~FreeTypeWrapper() { _state.reset(); }

int FreeTypeWrapper::drawString(const DrawInfo& info) {
    int berror = false;

    do {
        if (!info.utf8_str || !info.iv || info.x < 0 || info.y < 0 ||
            info.font_size < 0 || info.outline_width < 0.0) {
                ...
                if (face->glyph->format == FT_GLYPH_FORMAT_OUTLINE) {
                    ...
                    if (FT_Get_Glyph(face->glyph, &glyph) == 0) {
                        ...
                        if (!spans.empty()) {
                            ...
                            // This is unused in this test but you would need
                            // this to draw more than one glyph.
                            float bearingX =
                            ...
                            // Allocate data for our image and clear it out to
                            // transparent. Pixel32 *pxl = new Pixel32[imgSize];
                            // memset(pxl, 0xff, sizeof(Pixel32) * imgSize);
                            std::vector<Pixel32> pxl(imgSize);

                            // Loop over the outline spans and just draw them
                            // into the image.
                            for (Spans::iterator s = outlineSpans.begin();
                                 s != outlineSpans.end(); ++s) {
                                for (int w = 0; w < s->width; ++w) {
                                    std::size_t offset =
                                        (imgHeight - 1 - (s->y - rect.ymin)) *
                                            imgWidth +
                                        s->x - rect.xmin + w;
                                    if (offset >= pxl.size()) {
                                        xlog_err("offset error(%d,%d)",
                                                 (int)offset, (int)pxl.size());
                                        continue;
                                    }
                                    pxl[offset] =
                            ...
                            // Then loop over the regular glyph spans and blend
                            // them into the image.
                            for (Spans::iterator s = spans.begin();
                                 s != spans.end(); ++s) {
                                for (int w = 0; w < s->width; ++w) {
                                    ...
                                    Pixel32 src =
                                        Pixel32(fontCol.r, fontCol.g, fontCol.b,
                                                s->coverage);
                                    dst.r = (int)(dst.r +
                                                  ((src.r - dst.r) * src.a) /
                                                      255.0f);
                            ...
                            // WriteTGA(fileName, pxl, imgWidth, imgHeight);
                            {
                                xlog_trc(
                                    "[%04x]: "
                                    "[w=%d,h=%d][bx=%d,by=%d,ad=%d,as=%d]",
                                    (unsigned int)ref_utf32_c, imgWidth,
                                    imgHeight, (int)bearingX, (int)bearingY,
                                    (int)advance, (int)ascender);
                                ...
                                for (int iy = 0; iy < imgHeight; ++iy) {
                                    for (int ix = 0; ix < imgWidth; ++ix) {
                                        ...
                                        uint8_t color =
                                            pxl[offset].r;  // use red as color
                                        uint8_t alpha = pxl[offset].a;  //

                                        PixelColorPtr mid_fore_out =
                                            mid(info.outline, info.foreground,
                                                color);
                                        PixelColorPtr mid_foreout_back =
                                            mid(info.background, mid_fore_out,
                                                alpha);

                                        int realx = ix + xoff + bearingX;
                                        int realy =
                                            iy + info.y + ascender - bearingY;

                                        info.iv->drawPixels(realx, realy,
                                                            *mid_foreout_back);
                                    }
                                }
                            }
                        ...
                        // xoff += (face->glyph->advance.x >> 6) + ::ceil(2 *
                        // info.outline_width);
                        xoff += ((face->glyph->metrics.horiAdvance >> 6) +
                                 ::ceil(2 * info.outline_width));
                    }
                }
            }
        }
...
int FreeTypeWrapper::drawString_Normal_Monochrome(const DrawInfo& info) {
    ...
    do {
        ...
        for (int i = 0; i < (int)utf32_str.size(); ++i) {
            ...
            drawBitmap(info.iv, slot->bitmap_left,
...
std::shared_ptr<FreeTypeWrapper::State> FreeTypeWrapper::init() {
    ...
    do {
        ...
        err = FT_New_Face(state->ft_library, _font_path.c_str(), 0,
                          &state->ft_face);
        if (err) {
...
/* Here LeftTop --> (0,0) */
int FreeTypeWrapper::drawBitmap(
    std::shared_ptr<ImageView> iv, int x, int y, FT_Bitmap* bitmap,
    std::shared_ptr<std::vector<uint8_t>> foreground,
    std::shared_ptr<std::vector<uint8_t>> background) {
    xlog_trc("[x=%d,y=%d][w=%d, h=%d]", x, y, (int)bitmap->width,
             (int)bitmap->rows);
...
FreeTypeWrapper::PixelColorPtr FreeTypeWrapper::mid(PixelColorPtr background,
    ...
    do {
        ...
        for (std::size_t i = 0; i < pixel_bytes; ++i) {
            ...
            int middle = back + (fore - back) * (color - color_min) /
                                    (color_max - color_min);
