...
int XPlay::streamComponentOpen(int stream_index) {
    ...
    do {
        ...
        if (avcodec_parameters_to_context(
                avctx, ic->streams[stream_index]->codecpar) < 0) {
        ...
        switch (avctx->codec_type) {
            ...
            case AVMEDIA_TYPE_VIDEO: {
                ...
                if (_is->viddec.init(avctx, _is->videoq,
                                     _is->cond_continue_read_thread) < 0) {
                    berror = true;
...
int Decoder::init(
    AVCodecContext* avctx_tmp, std::shared_ptr<PacketQueue> queue_tmp,
...
int XPlay::readThread() {
    ...
    do {
        ...
        if (avformat_open_input(&ic, _is->filename.c_str(), nullptr, nullptr) <
            0) {
    ...
    if (!berror) {
        for (;;) {
            ...
            if (_is->videoq->size() > MAX_QUEUE_SIZE ||
                streamHasEnoughPackets(_is->video_st, _is->video_stream,
                                       _is->videoq)) {
                /* wait some time */
                std::unique_lock<std::mutex> lock(wait_mutex);
                xlog_trc("packet queue full, wait");
                _is->cond_continue_read_thread->wait_for(
...
static void SaveYUV420Frame(const AVFrame* pVFrame, FILE* fp) {
    SaveYUVBlock((unsigned char*)pVFrame->data[0], pVFrame->linesize[0],
                 pVFrame->width, pVFrame->height, fp);
    SaveYUVBlock((unsigned char*)pVFrame->data[1], pVFrame->linesize[1],
                 pVFrame->width / 2, pVFrame->height / 2, fp);
    SaveYUVBlock((unsigned char*)pVFrame->data[2], pVFrame->linesize[2],
                 pVFrame->width / 2, pVFrame->height / 2, fp);
}
...
static void SaveYUVNV12Frame(const AVFrame* pVFrame, FILE* fp) {
    SaveYUVBlock((unsigned char*)pVFrame->data[0], pVFrame->linesize[0],
                 pVFrame->width, pVFrame->height, fp);
    SaveYUVBlock((unsigned char*)pVFrame->data[1], pVFrame->linesize[1],
                 pVFrame->width, pVFrame->height / 2, fp);
}
...
int XPlay::queuePicture(AVFrame* src_frame, double pts, double duration,
    ...
    xlog_trc("w%d:h%d:pts%.2f:serial%d", vp->width, vp->height, vp->pts,
             vp->serial);
...
int XPlay::streamHasEnoughPackets(AVStream* st, int stream_id,
...
int XPlay::doRefresh(RefreshState* state) {
    if (_is->pictq->nb_remaining() == 0) {
        ...
        for (;;) {
            ...
            xlog_trc("serial(vp:%d,last_vp:%d,queue:%d)", vp->serial,
                     last_vp->serial, _is->videoq->serial());
            ...
            _is->refreshctx.frame_timer += delay;
            if (delay > 0 &&
                time - _is->refreshctx.frame_timer > AV_SYNC_THRESHOLD_MAX) {
    ...
    if (state->frame) {
        xlog_trc("state: [pts=%.2f, duration=%.2f, remaining_time=%.2f]",
                 state->frame->pts, state->frame->duration,
                 state->remaining_time);
    } else {
        xlog_trc("state: [null, remaining_time=%.2f]", state->remaining_time);
    }
...
double XPlay::compute_target_delay(double delay) { return delay; }