...
PcapngParser::PcapngParser() { _d = std::make_shared<InnerData>(); }

PcapngParser::~PcapngParser() {}

int PcapngParser::parse(const std::string& file,
                        std::function<int(const PcapngContent&)> callback) {
    bool error = false;
    ...
    do {
        ...
        for (;;) {
            ...
            for (auto it = shb->options->options.begin();
                 it != shb->options->options.end(); ++it) {
                switch (it->type) {
                    case SHB_OPTION_HARDWARE: {
                        content.info.shb_hardware.assign(
                            (const char*)it->data.data(), it->data.size());
                        break;
                    }
                    case SHB_OPTION_OS: {
                        content.info.shb_os.assign((const char*)it->data.data(),
                                                   it->data.size());
                        break;
                    }
                    case SHB_OPTION_USERAPPL: {
                        content.info.shb_userappl.assign(
                            (const char*)it->data.data(), it->data.size());
                        break;
                    }
                    default: {
            ...
            for (;;) {
                ...
                if (BT_IDB == block_probe->block_type) {
                    ...
                    for (auto it = idb->options->options.begin();
                         it != idb->options->options.end(); ++it) {
                        switch (it->type) {
                            case IDB_OPTION_NAME: {
                                content.interface.if_name.assign(
                                    (const char*)it->data.data(),
                                    it->data.size());
                                break;
                            }
                            case IDB_OPTION_DESCRIPTION: {
                                content.interface.if_description.assign(
                                    (const char*)it->data.data(),
                                    it->data.size());
                                break;
                            }
                            case IDB_OPTION_IPV4ADDR: {
                                ...
                                memcpy(arr.data(), it->data.data(),
                                       it->data.size());
                                content.interface.if_IPv4addrs.push_back(
                                    std::move(arr));
                                break;
                            }
                            case IDB_OPTION_IPV6ADDR: {
                                ...
                                memcpy(arr.data(), it->data.data(),
                                       it->data.size());
                                content.interface.if_IPV6addrs.push_back(
                                    std::move(arr));
                                break;
                            }
                            case IDB_OPTION_MACADDR: {
                                if (it->data.size() !=
                                    content.interface.if_MACaddr.size()) {
                                    xlog_err("option.mac size error");
                                    error = true;
                                    break;
                                }
                                memcpy(content.interface.if_MACaddr.data(),
                                       it->data.data(), it->data.size());
                                break;
                            }
                            case IDB_OPTION_EUIADDR: {
                                if (it->data.size() !=
                                    content.interface.if_EUIaddr.size()) {
                                    xlog_err("option.euiaddr size error");
                                    error = true;
                                    break;
                                }
                                memcpy(content.interface.if_EUIaddr.data(),
                                       it->data.data(), it->data.size());
                                break;
                            }
                            case IDB_OPTION_SPEED: {
                                if (it->data.size() !=
                                    sizeof(content.interface.if_speed)) {
                                    xlog_err("option.speed size error");
                                    error = true;
                                    break;
                                }
                                static_assert(
                                    sizeof(content.interface.if_speed) == 8,
                                    "size error");
                                content.interface.if_speed =
                                    to_uint64(info, it->data.data());
                                break;
                            }
                            case IDB_OPTION_TSRESOL: {
                                if (it->data.size() !=
                                    sizeof(content.interface.if_tsresol)) {
                                    xlog_err("option.tsresol size error");
                                    error = true;
                                    break;
                                }
                                static_assert(
                                    sizeof(content.interface.if_tsresol) == 1,
                                    "size error");
                                content.interface.if_tsresol =
                                    to_uint8(info, it->data.data());
                                break;
                            }
                            case IDB_OPTION_TZONE: {
                                if (it->data.size() !=
                                    sizeof(content.interface.if_tzone)) {
                                    xlog_err("option.tzone size error");
                                    error = true;
                                    break;
                                }
                                static_assert(
                                    sizeof(content.interface.if_tzone) == 4,
                                    "size error");
                                content.interface.if_tzone =
                                    to_uint32(info, it->data.data());
                                break;
                            }
                            case IDB_OPTION_OS: {
                                content.interface.if_os.assign(
                                    (const char*)it->data.data(),
                                    it->data.size());
                                break;
                            }
                            case IDB_OPTION_TSOFFSET: {
                                if (it->data.size() !=
                                    sizeof(content.interface.if_tsoffset)) {
                                    xlog_err("option.tsoffset size error");
                                    error = true;
                                    break;
                                }
                                static_assert(
                                    sizeof(content.interface.if_tsoffset) == 8,
                                    "size error");
                                content.interface.if_tsoffset =
                                    to_uint64(info, it->data.data());
                                break;
                            }
                            case IDB_OPTION_HARDWARE: {
                                content.interface.if_hardware.assign(
                                    (const char*)it->data.data(),
                                    it->data.size());
                                break;
                            }
                            case IDB_OPTION_TXSPEED: {
                                if (it->data.size() !=
                                    sizeof(content.interface.if_txspeed)) {
                                    xlog_err("option.txspeed size error");
                                    error = true;
                                    break;
                                }
                                static_assert(
                                    sizeof(content.interface.if_txspeed) == 8,
                                    "size error");
                                content.interface.if_txspeed =
                                    to_uint64(info, it->data.data());
                                break;
                            }
                            case IDB_OPTION_RXSPEED: {
                                if (it->data.size() !=
                                    sizeof(content.interface.if_rxspeed)) {
                                    xlog_err("option.rxspeed size error");
                                    error = true;
                                    break;
                                }
                                static_assert(
                                    sizeof(content.interface.if_rxspeed) == 8,
                                    "size error");
                                content.interface.if_rxspeed =
                                    to_uint64(info, it->data.data());
                                break;
                            }
                            default: {
...
std::shared_ptr<PcapngParser::Options> PcapngParser::doParserOptions(
    std::shared_ptr<PcapngParser::PcapngInfo> info, std::size_t options_size) {
    ...
    do {
        ...
        for (int i = 0;; ++i) {
            ...
            if (pos_now + (int64_t)minium_option_size >
                pos_initial + (int64_t)options_size) {
            ...
            int32_t option_padding_size =
                option_len_round_up_4 - option.option_len;
...
std::shared_ptr<PcapngParser::Block> PcapngParser::doProbeBlock(
    std::shared_ptr<PcapngInfo> info) {
    bool error = false;
    bool file_end = false;
...
int PcapngParser::doSkipBlock(std::shared_ptr<PcapngInfo> info) {
    ...
    do {
        ...
        uint32_t head_len =
            sizeof(block.block_type) + sizeof(block.block_length);
...
std::shared_ptr<PcapngParser::SectionHeaderBlock> PcapngParser::doParseSHB(
    std::shared_ptr<PcapngInfo> info) {
    bool error = false;
...
std::shared_ptr<PcapngParser::InterfaceDescriptionBlock>
PcapngParser::doParseIDB(std::shared_ptr<PcapngInfo> info) {
    bool error = false;
    ...
    do {
        ...
        /* size without options */
        constexpr uint32_t other_option_len =
            (sizeof(idb->block_type) + sizeof(idb->block_total_length) +
             sizeof(idb->link_type) + sizeof(idb->reserved) +
             sizeof(idb->snap_length) +
             sizeof(idb->block_total_length_trailing));
...
std::shared_ptr<PcapngParser::EnhancedPacketBlock> PcapngParser::doParseEPB(
    std::shared_ptr<PcapngInfo> info) {
    bool error = false;
    ...
    do {
        ...
        *epb->packet_data = info->fio->read(epb->captured_packet_length);
        ...
        uint32_t data_with_padding_4_size =
            round_up_4(epb->captured_packet_length);
        ...
        uint32_t other_option_len =
            sizeof(epb->block_type) + sizeof(epb->block_total_length) +
            sizeof(epb->interface_id) + sizeof(epb->timestamp_high) +
            sizeof(epb->timestamp_low) + sizeof(epb->captured_packet_length) +
            sizeof(epb->original_packet_length) + data_with_padding_4_size +
            sizeof(epb->block_total_length_trailing);
...
std::shared_ptr<PcapngParser::SimplePacketBlock> PcapngParser::doParseSPB(
    std::shared_ptr<PcapngInfo> info) {
    bool error = false;
    ...
    do {
        ...
        uint32_t other_data_size = sizeof(spb->block_type) +
                                   sizeof(spb->block_total_length) +
                                   sizeof(spb->original_packet_length) +
...
uint16_t PcapngParser::to_uint16(std::shared_ptr<PcapngInfo> info, void* data) {
    ...
    if (info->littleEndian) {
        value = (uint16_t)(pdata[0]) + ((uint16_t)(pdata[1]) << 8);
    } else {
        value = ((uint16_t)(pdata[0]) << 8) + (uint16_t)(pdata[1]);
    }
    return value;
}
...
uint32_t PcapngParser::to_uint32(std::shared_ptr<PcapngInfo> info, void* data) {
    ...
    uint8_t* pdata = (uint8_t*)data;
    if (info->littleEndian) {
        value = (uint32_t)(pdata[0]) + ((uint32_t)(pdata[1]) << 8) +
                ((uint32_t)(pdata[2]) << 16) + ((uint32_t)(pdata[3]) << 24);
    } else {
        value = ((uint32_t)(pdata[0]) << 24) + ((uint32_t)(pdata[1]) << 16) +
