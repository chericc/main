...
int PacketProcess::processEthernetData(
...
int PacketProcess::processEthernetPacket(std::shared_ptr<SharedPacket> packet) {
    ...
    do {
        ...
        std::shared_ptr<EthernetPacketInfo> eth =
            std::make_shared<EthernetPacketInfo>();
        ...
        std::size_t ethernet_header_size =
            eth->mac_dst.size() + eth->mac_src.size() + sizeof(eth->type);
        ...
        // mac_dst
        net_copy(eth->mac_dst.data(), eth->mac_dst.size(), pdata, size);

        // mac_src
        pdata += eth->mac_dst.size();
        size -= eth->mac_dst.size();
        net_copy(eth->mac_src.data(), eth->mac_src.size(), pdata, size);
        ...
        // integer limit check
        if (data_tmp.offset < data.offset || data_tmp.size > data.size) {
        ...
        data = data_tmp;
        packet->parsed_info.push_back(
            std::make_pair(PacketType::Ethernet, eth));
...
int PacketProcess::processIPv4Packet(std::shared_ptr<SharedPacket> packet) {
    ...
    do {
        std::shared_ptr<IPv4PacketInfo> ipv4 =
            std::make_shared<IPv4PacketInfo>();
        ...
        const std::size_t head_size_without_options =
            sizeof(ipv4->version_and_len) +
            sizeof(ipv4->differentialted_services_field) +
            sizeof(ipv4->total_length) + sizeof(ipv4->identification) +
            sizeof(ipv4->flag_and_fragment_offset) +
            sizeof(ipv4->time_to_live) + sizeof(ipv4->protocol) +
            sizeof(ipv4->header_checksum) + ipv4->ip_addr_src.size() +
        ...
        ipv4->fragment_offset =
            ipv4->ipv4_fragment_offset(ipv4->flag_and_fragment_offset);
        ...
        net_copy(ipv4->ip_addr_src.data(), ipv4->ip_addr_src.size(), pdata,
                 size);
        pdata += ipv4->ip_addr_src.size();
        size -= ipv4->ip_addr_src.size();

        net_copy(ipv4->ip_addr_dst.data(), ipv4->ip_addr_dst.size(), pdata,
                 size);
...
int PacketProcess::processUDPPacket(std::shared_ptr<SharedPacket> packet) {
    ...
    do {
        ...
        std::size_t udp_head_size =
            sizeof(udp->port_src) + sizeof(udp->port_dst) +
...
int PacketProcess::processRTPPacket(std::shared_ptr<SharedPacket> packet) {
    ...
    do {
        ...
        const std::size_t csrc_id_list_num =
            std::min(15u, (unsigned int)rtp->cc);
        ...
        // copy
        const std::size_t total_head_size =
            fixed_header_size + scrc_id_list_size + extended_header_size;
