...
std::string str_ipv4(const std::array<uint8_t, 4>& ipv4) {
    ...
    snprintf(str_tmp, sizeof(str_tmp),
             "%" PRIu8 ".%" PRIu8 ".%" PRIu8 ".%" PRIu8, ipv4[0], ipv4[1],
             ipv4[2], ipv4[3]);
...
std::string str_ipv6(const std::array<uint8_t, 16>& ipv6) {
    ...
    snprintf(str_tmp, sizeof(str_tmp),
             ...
             ipv6[7], ipv6[8], ipv6[9], ipv6[10], ipv6[11], ipv6[12], ipv6[13],
             ipv6[14], ipv6[15]);
...
std::pair<std::string, std::string> str_ipv4addr(
    ...
    snprintf(str_tmp, sizeof(str_tmp),
             "%" PRIu8 ".%" PRIu8 ".%" PRIu8 ".%" PRIu8, ipv4[4], ipv4[5],
             ipv4[6], ipv4[7]);
...
std::string str_euiaddr(const std::array<uint8_t, 8>& euiaddr) {
    ...
    snprintf(str_tmp, sizeof(str_tmp),
             ...
             euiaddr[0], euiaddr[1], euiaddr[2], euiaddr[3], euiaddr[4],
             euiaddr[5], euiaddr[6], euiaddr[7]);
    return std::string(str_tmp);
}
...
uint32_t net_u32(const void* ptr, std::size_t size) {
    ...
    return pi[3] + ((uint32_t)pi[2] << 8) + ((uint32_t)pi[1] << 16) +
           ((uint32_t)pi[0] << 24);
}
...
uint64_t net_u64(const void* ptr, std::size_t size) {
    ...
    return pi[7] + ((uint64_t)pi[6] << 8) + ((uint64_t)pi[5] << 16) +
           ((uint64_t)pi[4] << 24) + ((uint64_t)pi[3] << 32) +
           ((uint64_t)pi[2] << 40) + ((uint64_t)pi[1] << 48) +
           ((uint64_t)pi[0] << 56);
}
