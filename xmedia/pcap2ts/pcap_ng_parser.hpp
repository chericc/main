...
class PcapngParser : public XNonCopyableObject {
   public:
    ...
    int parse(const std::string& file,
              std::function<int(const PcapngContent&)>);
   ...
   private:
    ...
    std::shared_ptr<Options> doParserOptions(std::shared_ptr<PcapngInfo> info,
                                             std::size_t size);
    ...
    std::shared_ptr<InterfaceDescriptionBlock> doParseIDB(
        std::shared_ptr<PcapngInfo>);
    std::shared_ptr<EnhancedPacketBlock> doParseEPB(
        std::shared_ptr<PcapngInfo>);
