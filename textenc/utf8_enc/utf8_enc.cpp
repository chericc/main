...
std::string enc_utf8_substr(const std::string& utf8_in, size_t pos,
    ...
    return enc_utf32_2_utf8(
        enc_utf32_substr(enc_utf8_2_utf32(utf8_in), pos, count));
}
...
std::vector<uint32_t> enc_utf32_substr(const std::vector<uint32_t>& utf32_in,
    ...
    for (size_t i = pos; (i < pos + count) && (i < in_size); ++i) {
...
static int enc_unicode_to_utf8_one(uint32_t unic, char* pOutput, int outSize) {
    if (outSize < 6) {
        return 0;
    }
...
static int enc_utf8_to_unicode_one(const char* pInput, int utfbytes,
                                   uint32_t* Unic) {
...
std::vector<uint32_t> enc_utf8_2_utf32(const std::string& utf8_in) {
    ...
    while (in_index < max_len) {
        ...
        if (enc_utf8_to_unicode_one(utf8_in.data() + in_index, utf8bytes,
                                    &utf32_value) > 0) {
...
std::string enc_utf32_2_utf8(const std::vector<uint32_t>& utf32_in) {
    ...
    for (auto const& utf32_value : utf32_in) {
        ...
        int ret = enc_unicode_to_utf8_one(utf32_value, utf8_buffer,
